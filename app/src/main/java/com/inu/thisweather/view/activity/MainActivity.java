...
import static com.inu.thisweather.util.Utils.getNoPoint;
    ...
    private void getLocalForecast() {
        ...
        localForecast.enqueue(new Callback<JsonObject>() {
            ...
            public void onResponse(Call<JsonObject> call, Response<JsonObject> response) {
                ...
                Log.d("MainActivity","local, " + array);
                setMain(array);
            }
            @Override
            public void onFailure(Call<JsonObject> call, Throwable t) {
                Log.d("MainActivity","local, " + t +"");
                setMain(null);
            }
        });
    }
...
//                overridePendingTransition(0, 0);
    ...
    private void setMain(JsonArray array) {
        ...
        if (array != null) {
            ...
            Log.d("MainActivity", data[2]);
            Log.d("MainActivity", data[3]);
            Log.d("MainActivity", data[4]);
            Log.d("MainActivity", data[5]);
        }
        else{
            for (int i = 0; i < 6; i++){
                data[i] = "";
    ...
    private void setLionHead(String data) {
        ...
        switch (data){
            ...
            default: {
                ...
                rain.setVisibility(View.INVISIBLE);
                dust.setVisibility(View.INVISIBLE);
                Toast.makeText(MainActivity.this, "잠시 후 다시 시도해주세요.", Toast.LENGTH_LONG).show();
                break;
            }
        }
    }
